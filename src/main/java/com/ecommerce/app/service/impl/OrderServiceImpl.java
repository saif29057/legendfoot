...
public class OrderServiceImpl implements OrderService {
    ...
    // NOTE: Removed CartService dependency to break circular dependency cycle
    ...
    public Order createOrderFromCart(User user) {
        ...
        // ✅ FIXED: Get user's active cart
        Cart cart = cartRepository.findActiveCartByUser(user)
                .orElseThrow(() -> new RuntimeException("No active cart found for user: " + user.getUsername()));
        ...
        // ✅ FIXED: Create OrderItems from CartItems
        ...
        // ✅ FIXED: Create order with calculated total
        Order order = new Order();
        order.setUser(user);
        order.setOrderDate(LocalDateTime.now());
        order.setStatus(Order.OrderStatus.PENDING);
        order.setTotalPrice(totalPrice); // ✅ Set to actual cart total, not zero

        Order savedOrder = orderRepository.save(order);
        log.info("Successfully created order with ID: {} and {} items", savedOrder.getId(), orderItems.size());

        // ✅ FIXED: Save order items
    ...
    public OrderDto getOrderById(Long id) {
        ...
        if (order.isPresent()) {
            Order o = order.get();
            // ✅ Force loading of lazy-loaded orderItems within transaction
            // This ensures items are available when converting to DTO
            o.getOrderItems().size();  // Trigger lazy load
